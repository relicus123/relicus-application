...
import { CAREERS } from "../../../constants/knowNext/careers";
...
import { INDUSTRIES } from "../../../constants/knowNext/industries";
...
export function GlobalSearch({ onNavigate }: GlobalSearchProps) {
  ...
  const results = useMemo(() => {
    ...
    const careers = CAREERS.filter((c) => c.title.toLowerCase().includes(lower))
      .map((c) => ({ id: c.id, title: c.title, type: "career" as const, subtitle: c.category, icon: "🧬" }));
    const colleges = COLLEGES.filter((c) => c.name.toLowerCase().includes(lower) || c.shortName.toLowerCase().includes(lower))
      .map((c) => ({ id: c.id, title: c.name, type: "college" as const, subtitle: c.location, icon: "🏛️" }));
    const scholarships = SCHOLARSHIPS.filter((s) => s.name.toLowerCase().includes(lower))
      .map((s) => ({ id: s.id, title: s.name, type: "scholarship" as const, subtitle: s.provider, icon: "🎓" }));
    return [...careers, ...colleges, ...scholarships].slice(0, 10);
  }, [query]);
...
export const LandingHub: React.FC<LandingHubProps> = ({ onNavigate, onBack }) => {
  ...
  const stats = [
    { label: "Careers", value: CAREERS.length, icon: "🧬" },
    { label: "Colleges", value: COLLEGES.length, icon: "🏛️" },
    { label: "Scholarships", value: SCHOLARSHIPS.length, icon: "🎓" },
...
export const CareerPlan: React.FC<CareerPlanProps> = ({ onNavigate, onBack }) => {
  ...
  const careerGoal = useMemo(() => CAREERS.find((c) => c.id === careerGoalId), [careerGoalId]);
...
export const SavedItems: React.FC<SavedItemsProps> = ({ onNavigate, onBack }) => {
  ...
  const careers = useMemo(() => CAREERS.filter((c) => savedCareerIds.includes(c.id)), [savedCareerIds]);
  const colleges = useMemo(() => COLLEGES.filter((c) => savedCollegeIds.includes(c.id)), [savedCollegeIds]);
  const scholarships = useMemo(() => SCHOLARSHIPS.filter((s) => savedScholarshipIds.includes(s.id)), [savedScholarshipIds]);
