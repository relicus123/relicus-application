...
export default function Home() {
  ...
  const activeCourse = useMemo(() => {
    if (!enrolledCourseIds || enrolledCourseIds.length === 0) return null;
    return courses
      .filter((c) => enrolledCourseIds.includes(c.id))
      .find((c) => {
        const completed = Object.keys(lessonProgress).filter(
          (key) => key.startsWith(`${c.id}_`) && lessonProgress[key].completed
        ).length;
        const total = c.modules.flatMap((m) => m.lessons).length;
        const pct = total > 0 ? Math.round((completed / total) * 100) : 0;
        return pct < 100;
      }) || courses.find((c) => enrolledCourseIds.includes(c.id));
  }, [enrolledCourseIds, courses, lessonProgress]);

  const activeCourseProgress = useMemo(() => {
    if (!activeCourse) return 0;
    const completed = Object.keys(lessonProgress).filter(
      (key) => key.startsWith(`${activeCourse.id}_`) && lessonProgress[key].completed
    ).length;
    const total = activeCourse.modules.flatMap((m) => m.lessons).length;
    return total > 0 ? Math.round((completed / total) * 100) : 0;
  }, [activeCourse, lessonProgress]);
