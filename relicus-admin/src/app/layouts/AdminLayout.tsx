...
export function AdminLayout() {
  ...
  useEffect(() => {
    async function checkAuth() {
      try {
        console.log("[AdminLayout] 1. Checking auth...");
        if (!supabase || !supabase.auth) {
          console.error("[AdminLayout] Supabase not fully configured.");
          setAuthorized(false);
          setLoading(false);
          return;
        }
        ...
        console.log("[AdminLayout] 2. Got session:", { hasSession: !!session, userId: session?.user?.id });
        if (!session) {
          navigate("/admin/login");
          return;
        }

        console.log("[AdminLayout] 3. Fetching profile for user ID:", session.user.id);
        const { data: profile, error } = await supabase
          .from("profiles")
          .select("role, id, email")
          .eq("id", session.user.id)
          .single();
        console.log("[AdminLayout] 4. Profile query result:", { profile, error });

        if (error || !profile || profile.role !== "admin") {
          // Not an admin
          console.log("[AdminLayout] 5. Not an admin!");
          setAuthorized(false);
        } else {
          console.log("[AdminLayout] 5. User is admin!");
          setAuthorized(true);
