...
export function TeacherDashboard() {
  ...
  const [statsData, setStatsData] = useState({ totalStudents: 0, classesToday: 0, pendingReviews: 0 });
    ...
    async function loadData() {
      try {
        const [classesRes, assignmentsRes, studentsRes] = await Promise.all([
          ...
          supabase.from("tuition_students").select("*")
        ]);

        if (classesRes.data) setUpcomingClasses(classesRes.data);
        if (assignmentsRes.data) setAssignments(assignmentsRes.data);
        if (studentsRes.data) setRecentStudents(studentsRes.data);
        
        setStatsData({
          totalStudents: studentsRes.data?.length || 0,
          classesToday: classesRes.data?.length || 0,
          pendingReviews: assignmentsRes.data?.reduce((acc: number, a: any) => acc + (a.pending || 0), 0) || 0
        });
  ...
  const stats = [
    { label: "Total Students", value: statsData.totalStudents, icon: Users, color: "from-primary to-secondary" },
    { label: "Classes Today", value: statsData.classesToday, icon: Calendar, color: "from-secondary to-accent" },
    { label: "Pending Reviews", value: statsData.pendingReviews, icon: FileText, color: "from-accent to-primary" },
  ];
      ...
      <div className="px-6 -mt-4 space-y-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {upcomingClasses.length === 0 && <p className="text-sm text-muted-foreground">No classes scheduled.</p>}
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">{cls.grade} • {cls.students_count || 0} students</p>
                  </div>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-muted-foreground">
                {assignments.reduce((acc, a) => acc + (a.pending || 0), 0)} to review
              </span>
            </div>
          </CardHeader>
          <CardContent className="space-y-3">
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className="px-2 py-1 bg-accent/20 text-primary text-sm font-semibold rounded-lg">
                    {assignment.submitted_count || 0}/{assignment.total || 1}
                  </span>
                </div>
                <div className="flex items-center gap-2 mb-2">
                  <div className="flex-1 h-2 bg-border rounded-full overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${((assignment.submitted_count || 0) / (assignment.total || 1)) * 100}%` }}
                      transition={{ delay: index * 0.1, duration: 0.8 }}
                      className="h-full bg-gradient-to-r from-primary to-secondary rounded-full"
                    />
                  </div>
                  <span className="text-xs text-muted-foreground">
                    {Math.round(((assignment.submitted_count || 0) / (assignment.total || 1)) * 100)}%
                  </span>
                </div>
                {(assignment.pending || 0) > 0 && (
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {recentStudents.length === 0 && <p className="text-sm text-muted-foreground">No students enrolled.</p>}
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Attendance: {Math.round((student.attendance || 0) / 50 * 100)}%</p>
                  </div>
                </div>
